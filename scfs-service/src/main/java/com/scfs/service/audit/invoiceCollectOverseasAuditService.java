 ...
 * <pre>
 * 
 *  File: invoiceCollectOverseasAuditService.java
 *  Description: Invoice收票审核
 *  TODO
 *  Date,					Who,				
 *  201年08月03日			Administrator
 *
 * </pre>
 */
@Service
public class invoiceCollectOverseasAuditService extends AuditService {
	@Autowired
	...
	public void sendMessage(AuditNode auditNode, ProjectItemReqDto poAuditReqDto, Audit audit, Integer newId) {
		if (null != auditNode) { // 中间审核节点
			...
			sendWarnRtx(audit.getAuditorId(), "invoice收票申请审核单据", "Invoice收票申请财务专员审核通过");
		}
	}
	 ...
	 * @return
	 */
	public Result<InvoiceCollectOverseasAuditInfo> queryCollectOverseasAuditInfo(Integer invoiceId) {
		Result<InvoiceCollectOverseasAuditInfo> result = new Result<InvoiceCollectOverseasAuditInfo>();
		InvoiceCollectOverseasResDto collectOverseas = invoiceCollectOverseasService
				.queryInvoiceCollectOverEntityResDto(invoiceId);
		List<InvoiceCollectOverseasFeeResDto> invoiceCollectFeeList = invoiceCollectOverseasFeeService
				.queryInvoiceCollectOverseasByResut(invoiceId);
		List<InvoiceCollectOverseasPoResDto> invoiceCollectPoList = invoiceCollectOverseasPoService
				.queryInvoiceCollectByOverId(invoiceId);
		List<InvoiceCollectOverseasPoFileResDto> invoiceCollectFileList = invoiceCollectOverseasPoService
				.queryFileList(invoiceId);
		InvoiceCollectOverseasAuditInfo auditInfo = new InvoiceCollectOverseasAuditInfo();
		auditInfo.setOverseasResDto(collectOverseas);
		auditInfo.setOverseasFeeResDto(invoiceCollectFeeList);
		auditInfo.setOverseasPoResDto(invoiceCollectPoList);
	...
	public void passFinanceAudit(ProjectItemReqDto poAuditReqDto) {
		...
		if (audit.getAuditType() == BaseConsts.ONE || audit.getAuditType() == BaseConsts.TWO) {
			...
			AuditNode nextAuditNode = auditFlowService.getNextAuditNode(BaseConsts.INT_25, audit.getPoType(),
					audit.getProjectId());
			// 业务逻辑处理
			updateInvoiceCollectOverseasState(audit.getPoId(), this.getState(nextAuditNode)); // 待认证
	...
	public void unPassAudit(ProjectItemReqDto poAuditReqDto) {
		...
		if (audit.getAuditType() == BaseConsts.ONE || audit.getAuditType() == BaseConsts.TWO) {
			super.closeSighAudit(audit.getId());
			updateInvoiceCollectOverseasState(audit.getPoId(), BaseConsts.ONE); // 待提交
			sendMailResult(audit.getPoId(), audit.getAuditorId(), BaseConsts.TWO); // 发送邮件
			sendWarnRtx(audit.getAuditorId(), "Invoice收票申请审核单据", "Invoice收票申请审核不通过");
		}
	}
	...
	public void sighAudit(ProjectItemReqDto poAuditReqDto) {
		...
		sendWechatMsg(newId, "您有加签的Invoice收票单审核【" + newAudit.getPoNo() + "】");
	}
	...
	public void deliverAudit(ProjectItemReqDto poAuditReqDto) {
		...
		Audit newAudit = auditDao.queryAuditById(newId);
		sendWechatMsg(newId, "您有转交的Invoice收票审核【" + newAudit.getPoNo() + "】");
	}
	 ...
	 */
	public void startAudit(InvoiceCollectOverseas collectOverseas, AuditNode startAuditNode) {
		Audit audit = new Audit();
		audit.setSupplierId(collectOverseas.getSupplierId());
		audit.setPoDate(collectOverseas.getInvoiceDate());
		audit.setPoId(collectOverseas.getId());
		audit.setPoNo(collectOverseas.getApplyNo());
		audit.setProjectId(collectOverseas.getProjectId());
		audit.setBusinessUnitId(collectOverseas.getBusinessUnit());
		audit.setAmount(collectOverseas.getInvoiceAmount());
		...
		audit.setPoType(BaseConsts.INT_26); // 26-Invoice收票
		...
		sendMailMsg(collectOverseas.getId(), audit.getAuditorId(), BaseConsts.ONE); // 发送邮件
		sendWechatMsg(newId, "您有新的Invoice收票申请单审核【" + audit.getPoNo() + "】");
	}
	...
	public PageResult<AuditFlowsResDto> queryAuditFlows(Integer projectItemId) {
		...
		Integer poType = BaseConsts.INT_26;
	...
	private void sendMailMsg(Integer invoiceId, Integer auditId, Integer auditType) {
		BaseUser fromUser = ServiceSupport.getUser();
		InvoiceCollectOverseas collectOverseas = invoiceCollectOverseasService
				.queryInvoiceCollectOverEntityById(invoiceId);
		...
		mail1.setColumnOne("Invoice收票单编号");
		mail1.setColumnTwo(collectOverseas.getApplyNo());
		...
		String content = msgContentService.convertMailOneContent("Invoice收票收票单财务审核" + auditTypeStr, templateOnes, null);
		sendWarnMail(toUser.getId(), "Invoice收票收票单财务审核", content);
	}

	private void sendMailResult(Integer invoiceId, Integer auditId, Integer auditState) {
		InvoiceCollectOverseas collectOverseas = invoiceCollectOverseasService
				.queryInvoiceCollectOverEntityById(invoiceId);
		BaseUser toUser = cacheService.getUserByid(auditId);
		List<MailTemplateOne> templateOnes = Lists.newArrayList();
		...
		templateOnes.add(mail2);
		templateOnes.add(mail3);
		String content = msgContentService.convertMailOneContent("Invoice收票收票单财务审核结果", templateOnes, null);
		sendWarnMail(toUser.getId(), "Invoice收票收票单财务审核", content);
	}
	 ...
	 * @param state
	 */
	public void updateInvoiceCollectOverseasState(Integer id, Integer state) {
		InvoiceCollectOverseas collectOverseas = new InvoiceCollectOverseas();
		collectOverseas.setId(id);
		collectOverseas.setState(state);
		invoiceCollectOverseasService.updateInvoiceCollectOverseasById(collectOverseas);
	}
}
