 ...
 */
@Service
public class PoReturnAuditService extends AuditService {
	@Autowired
	...
	public void sendMessage(AuditNode auditNode, PoAuditReqDto poAuditReqDto, Audit audit, Integer newId) {
		if (null != auditNode) { // 中间审核节点
			sendMailMsg(newId);// 发送邮件提醒
			sendRTXMsg(newId); // 发送RTX
			sendWechatMsg(newId, "您有新的采购退货单审核【" + audit.getPoNo() + "】");
		} else { // 最后一个审核节点
	...
	public void startAudit(PurchaseOrderTitle purchaseOrderTitle, AuditNode startAuditNode) {
		...
		audit.setPoType(BaseConsts.INT_18);
	...
	public void passFinanceAudit(PoAuditReqDto poAuditReqDto) {
		...
		if (audit.getAuditType() == BaseConsts.ONE || audit.getAuditType() == BaseConsts.TWO) {
			...
			updatePoStateById(audit.getPoId(), getState(nextAuditNode), true);
	 ...
	 * 财务主管审核通过
	 */
	public void passFinance2Audit(PoAuditReqDto poAuditReqDto) {
		Audit audit = auditDao.queryAuditById(poAuditReqDto.getAuditId());
	...
	public void unPassAudit(PoAuditReqDto poAuditReqDto) {
		...
		if (audit.getAuditType() == BaseConsts.ONE || audit.getAuditType() == BaseConsts.TWO) {
			super.closeSighAudit(audit.getId());
			updatePoStateById(audit.getPoId(), BaseConsts.ONE, false);// 返回编辑状态
			sendMailMsgUnPass(audit);
		}
	}

	private void sendMailMsgUnPass(Audit audit) {
		...
		String content = msgContentService.convertMailOneContent("采购退货单单据转交提醒", templateOnes, null);
		sendWarnMail(audit.getAuditorId(), "采购退货单业务审核单据", content);
		// sendWarnRtx(audit.getAuditorId(), "采购退货单业务审核单据", "单据由" +
		// audit.getAuditor() + "审核不通过");
	}
	...
	public void sighAudit(PoAuditReqDto poAuditReqDto) {
		...
		sendWechatMsg(newId, "您有转交的采购退货单审核【" + newAudit.getPoNo() + "】");
	}
	...
	private void updatePoStateById(Integer poId, int state, boolean passAudit) {
		PurchaseOrderTitle purchaseOrderTitle = new PurchaseOrderTitle();
		purchaseOrderTitle.setState(state);// 更新采购退货单状态
		purchaseOrderTitle.setId(poId);
		purchaseOrderService.updatePurchaseOrderTitle(purchaseOrderTitle);

		BillOutStore billOutStore = billOutStoreService.queryValidBillOutStoreByPoReturnId(poId);
		if (passAudit == true) { // 审核通过
			BillOutStore billOutStore2 = new BillOutStore();
			billOutStore2.setId(billOutStore.getId());
			billOutStore2.setStatus(state);
			billOutStoreService.updateBillOutStoreById(billOutStore2);
		} else {
			// 作废出库单
	...
	private void sendMailMsg(int auditId) {
		...
		PoTitleRespDto poTitleRespDto = purchaseOrderService.queryPurchaseOrderTitleById(newAudit.getPoId()).getItems();
		String auditTypeStr = "";
		...
		mail5.setColumnOne("采购退货金额");
		mail5.setColumnTwo(
				DecimalUtil.toAmountString(poTitleRespDto.getOrderTotalAmount()) + poTitleRespDto.getCurrencyName());
		...
		String content = msgContentService.convertMailOneContent(auditTypeStr + "采购退货单" + typeStr, templateOnes, null);
		sendWarnMail(toUser.getId(), "SCFS系统提醒您,有新的" + auditTypeStr + "【采购退货单】需要审核", content);
	}
	...
	private void sendRTXMsg(int auditId) {
		...
		content = content + "采购退货单编号:" + poTitleRespDto.getOrderNo() + "\n";
		content = content + "项目:" + poTitleRespDto.getProjectName() + "\n";
		content = content + "客户:" + poTitleRespDto.getCustomerName() + "\n";
		content = content + "采购退货金额:" + DecimalUtil.toAmountString(poTitleRespDto.getOrderTotalAmount())
				+ poTitleRespDto.getCurrencyName() + "\n";
		if (!auditTypeStr.equals("")) {
		...
		sendWarnRtx(toUser.getId(), "SCFS系统提醒您,有新的" + auditTypeStr + "【采购退货单】需要审核", content);
	}
	...
	public PageResult<AuditFlowsResDto> queryAuditFlows(Integer projectItemId) {
		...
		Integer poType = BaseConsts.INT_18;
