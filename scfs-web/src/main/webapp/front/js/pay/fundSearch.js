...
window.operateEvents = {
    ...
    'click .edit': function(e, value, row, index){
		...
		GLOBAL.go("html/pay/request/fundBaseInfoEdit.html");
    },
    'click .scan': function(e, value, row, index){
		...
		options.param.payNo = row.payNo;
		options.param.stateInt = row.stateInt;
		GLOBAL.go("html/pay/request/fundBaseInfoScan.html");
    },
    'click .print': function(e, value, row, index){
		if(row.printNum > 0){
			layer.confirm("【" + row.payNo + "】已经打印过 " + row.printNum + "次，是否继续打印？",function(index) {
				var id = row.id;
				window.open("html/pay/request/fundPrint.html?id="+id);
				layer.close(index);
			});
		} else {
			var id = row.id;
				...
				if (hint.length > 0) {
					layer.confirm(hint + "是否继续打印？",function(index) {
						GLOBAL.ajax("payOrder/preBatch/print", JSON.stringify(param) , function(e) {
							if (e.success) {
								window.open("html/pay/request/fundBatchPrint.html="+ ids.unique().join(','));
							} else {
								layer.msg(e.msg);
							}
						},true);
					...
					GLOBAL.ajax("payOrder/preBatch/print", JSON.stringify(param) , function(e) {
						if (e.success) {
							window.open("html/pay/request/fundBatchPrint.html?ids="+ ids.unique().join(','));
						} else {
							layer.msg(e.msg);
						}
					},true);
