...
window.operateEvents = {
	    ...
	    if(id){
	    	layer.confirm('确定要提交吗？', {
		        btn: ['确定', '取消'] //按钮
		         ...
		         GLOBAL.ajax("fee/manage/submit", {id: id}, function(e) {
	                if (e.success) {
	                    layer.msg("提交成功！");
	                    $("#btnSearch").click();
	                } else {
    ...
    'click .delete': function (e, value, row, index) {
	    ...
	    layer.confirm('确定要删除吗？', {
	    	...
	    	GLOBAL.ajax("fee/manage/delete", {id: id}, function(){
	    		layer.msg("删除成功", {
	                icon: 1
	            })
	            $("#btnSearch").click();
    ...
    'click .edit': function(e, value, row, index){
		...
		GLOBAL.go("html/fee/manageFee/feeManageEdit.html", options.param);
    },
    'click .scan': function(e, value, row, index){
		options.param.id = row.id;
		options.param.state = row.state;
		GLOBAL.go("html/fee/manageFee/feeManageScan.html", options.param);
    },
    
    'click .share': function(e, value, row, index){
		options.param.id = row.id;
		GLOBAL.go("html/fee/manageFee/feeManageShare.html", options.param);
    }
...
$("#btnExport").click(function() {
	var count_url = "/feeManage/excel/export/count";
	var data = $("#searchForm").serializeObject();
	GLOBAL.ajax(count_url, data, function(e) {
        if (e.success) {
        	var url = "fee/excel/feeManage.xls";
...
//合并管理费用自动分摊
$("#feeManageConformityShare").click(function(){
	var ids=[];
